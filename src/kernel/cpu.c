...
struct timer my_timer[NCPU];
static void my_handler(struct timer *t) {
    // printk("CPU %d:Myhandler occurred\n", cpuid());
    // printk("Begin Here\n");
    // _arch_disable_trap();
    // printk("After Here\n");
    t->data++;
    _acquire_sched_lock();
    // set_cpu_timer(&my_timer[cpuid()]);
    // printk("CPU %d:handlerlock\n", cpuid());
    _sched(RUNNABLE);
}
...
define_early_init(init_timer) {
    for (int i = 0; i < NCPU; i++) {
        my_timer[i].elapse = 15;
        my_timer[i].handler = my_handler;
        my_timer[i].data = i * 100;
        my_timer[i].triggered = true;
    }
}
void set_cpu_on() {
    ...
    set_cpu_timer(&hello_timer[cpuid()]);
    // my_timer[cpuid()].elapse = 5;
    // my_timer[cpuid()].handler = my_handler;
    // my_timer[cpuid()].data = cpuid() * 100;
    // set_cpu_timer(&my_timer[cpuid()]);
}

void set_cpu_off() {
    bool s = _arch_disable_trap();
    if (s == true) {
    }
