...
typedef struct {
    ...
    /**
        ...
        @note distinguish it from `block_no` in `Block`, which is the "block
       number".

        `inode_no` should be the offset in block from the beginning of the inode
       area.
     */
    usize inode_no;
...
typedef struct {
    ...
    /**
        ...
        This method should be called before any write operation to `inode` and
       its file content.
    ...
    /**
       ...
       inode.
        ...
        If `do_write` is true and the inode is valid, write the content of
       `inode` to disk.

        If `do_write` is false and the inode is invalid, read the content of
       `inode` from disk.
    ...
    /**
        ...
        @note do not forget to reset related metadata of `inode`, e.g.
       `inode->entry.num_bytes`.
    ...
    /**
        ...
        "No one needs it" means it is useless BOTH in-memory (`inode->rc == 0`)
       and on-disk
        (`inode->entry.num_links == 0`).

        "Free the inode" means freeing all related file blocks and the inode
       itself.

        @note do not forget `kfree(inode)` after you have done them all!

        @note caller must NOT hold the lock of `inode`. i.e. caller should have
       `unlock`ed it.
    ...
    /**
        @brief read `count` bytes from `inode`, beginning at `offset`, to
       `dest`.
    ...
    /**
        ...
        Add a new directory entry in `inode` called `name`, which points to
       inode with `inode_no`.

        @return the index of new directory entry, or -1 if `name` already
       exists.

        @note if the directory inode is full, you should grow the size of
       directory inode.

        @note you do NOT need to change `inode->entry.num_links`. Another
       function to be finished in our final lab will do this.
    ...
    /**
        ...
        @note if the last entry is removed, you can shrink the size of directory
       inode. If you like, you can also move entries to fill the hole.
...
typedef struct free_inode_node {
    usize inode_no;
    struct free_inode_node *next;
} free_inode_node;
