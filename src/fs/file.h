...
struct ftable {
    ...
    // 创新点：有一个file_count，记录当前系统中打开的文件数目，只有大于NFILE时，才会去遍历
    File files[NFILE];
    int file_count;
    SpinLock lock;
};
...
/**
    @brief find an unused (i.e. ref == 0) file in the global file table and set
   ref to 1.
...
/**
    ...
    @note since `cache.end_op` may sleep, you should not hold any lock (I mean,
